...
{
    public class SwaggerJsonTransformerCustom : ISwaggerJsonTransformer
    {
        ...
        public SwaggerJsonTransformerCustom()
        {
            _httpMethods = new Lazy<HashSet<string>>(()
                => new HashSet<string>(DefaultMethodsTypes,
        ...
        private bool RemoveMethods(JProperty path, ReRouteOptions reRoute)
        {
            ...
            while (method != null)
            {
                if (!_httpMethods.Value.Contains(method.Name))
                {
